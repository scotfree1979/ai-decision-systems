#!/usr/bin/env python3
"""
Race Test: Volatility Spike / Whipsaw

Purpose
-------
Simulate a rapid flip (drift→steam) and verify volatility check logic engages
and the engine avoids unsafe entries or flips position per policy.
"""
...
def run(db_paths, provide, deps) -> Result:
    ...
    # drift then sharp steam
    provide.oc_band_samples(mid, sid, 1, [6.0, 6.1, 6.2, 6.3])
    provide.oc_band_samples(mid, sid, 2, [6.2, 6.0, 5.8, 5.6])

    # evaluate with volatility check present (best-effort)
    try:
        _ = deps.volatility_check  # ensure import ok
    ...
    # Accept either: guarded (no order) or an order with later narrative explaining flip/guard
    con = sqlite3.connect(db_paths.autoscalp, timeout=5); con.row_factory = sqlite3.Row
    try:
        o = con.execute("SELECT id, side, entry_status FROM orders ORDER BY id DESC LIMIT 1").fetchone()
        d = con.execute("SELECT signal_type, confidence FROM decisions ORDER BY id DESC LIMIT 1").fetchone()
    finally:
        try: con.close()
        except Exception: pass

    return Result.pass_(order=(dict(o) if o else None), decision=(dict(d) if d else None)).add("volatility spike handled per policy")
