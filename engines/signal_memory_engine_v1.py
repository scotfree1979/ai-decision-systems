...
class SignalMemoryEngine:
    ...
    def update_runner_memory(self, market_id, selection_id, updates: dict):
        ...
        snapshot = get_static_snapshot().get(key, {})
            ...
            for market_id, runners in list(self.live_markets.items()):
                for selection_id in runners:
                    try:
                        ...
                        snapshot["oc_snapshots"]["OC7"]["odds_series"].append(lay)
                        save_snapshot_to_ram(market_id, selection_id, snapshot)
    ...
    def load_oc_snapshots_from_db(self):
        ...
        print(f"✅ [load_oc_snapshots_from_db] Completed.")
    ...
    def generate_signal_status_report(self):
        ...
        while self.session_active or self.active_tradable_markets:
            try:
                ...
                ready = awaiting_tick = inside_window = forming = blocked = total_oc_snapshots = 0
                ...
                print("==========================================================")
                print(f"• Signals Generated:            {getattr(self, 'debug_signal_counter', 0)}")
                print(f"• Runners Monitored:        {total}")
                print(f"• Memory Synced Runners:    {len(static_snapshot)}")
...
def evaluate_active_runners(self):
    ...
    while True:
        try:
            memory_snapshot = self.get_static_snapshot()
    ...
    def ingest_market_snapshot(self, market_id, runners):
        ...
        # ✅ Ensure live_markets is populated
        if market_id not in self.live_markets:
        ...
        for r in runners:
            ...
            if pattern_key:
                ...
                signal.update({
                    ...
                    "scalp_ticks": 2,
                    "forced": True,
                    "customerOrderRef": f"{pattern_key}_{uuid.uuid4().hex[:6]}"
                })
                self.track_unmatched_signal(signal)
    ...
    def track_unmatched_signal(self, signal):
        ...
        if "conflict_flag" in signal:
            snapshot["conflict_flag"] = signal["conflict_flag"]
            snapshot.setdefault("snapshot", {})["conflict_flag"] = signal["conflict_flag"]
        ...
        from signal_memory_db_writer import save_signal_to_stm
