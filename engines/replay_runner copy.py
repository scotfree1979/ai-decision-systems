...
def _derive_surface_type(mid: str) -> str:
    """Return 'flat' or 'jumps' based on race_name keywords."""
    try:
        con = sqlite3.connect(bets_db()); con.row_factory = sqlite3.Row
        row = con.execute("SELECT race_name FROM bets WHERE marketId=? LIMIT 1", (mid,)).fetchone()
        con.close()
        txt = (row["race_name"] or "").lower() if row else ""
        if any(k in txt for k in ("chase","chs","hurdle","hrd","nhf")):
            return "jumps"
        return "flat"
    except Exception:
        return "unk"
...
# 📍 TARGET: engines/replay_runner.py
# 🔎 SEARCH: def _load_days(limit=None):
# 📆 PATCHED: 2025-10-07T15:05Z
import datetime as dt


def _load_days(limit=None):
    """
    Load the most recent N distinct days that have data in oc_series.
    Falls back to bets if oc_series has none. Returns ascending order.
    """
    con = sqlite3.connect(bets_db()); con.row_factory = sqlite3.Row
    try:
        # pull from oc_series first
        rows = con.execute("""
            SELECT DISTINCT date(snapshot_ts) AS d
            FROM oc_series
            WHERE d IS NOT NULL
            ORDER BY d DESC
            LIMIT ?
        ...
        # fallback if oc_series empty
        if not days:
            rows2 = con.execute("""
                SELECT DISTINCT date(marketStartTime) AS d
                FROM bets
                WHERE d IS NOT NULL
                ORDER BY d DESC
                LIMIT ?
        ...
        # ensure chronological ascending order for replay
        days.sort()
        if days:
            print(f"[replay] Selected days ({len(days)}): {days[0]} → {days[-1]}")
        else:
            print("[replay] No valid days found with data.")
        return days
    finally:
        con.close()
...
# 📆 PATCHED: 2025-10-02
# ───────────────────────────────────────────────────────────────
def run_replay(days:int=1, speed_x:float=60.0, epochs:int=1,
               start_balance:float=1000.0):

    """
    Replay past market days at compressed speed.
    """
    # attach all three DBs for cross-queries
    autodb = sqlite3.connect(autoscalp_db()); autodb.row_factory = sqlite3.Row
    autodb.execute("ATTACH 'data/bets.db' AS betsdb")
    autodb.execute("ATTACH 'data/settlements.db' AS setdb")
  
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 📍 TARGET: engines/replay_runner.py
# 🔎 SEARCH: bank_state.reset_for_replay(start_balance)
# 📆 PATCHED: 2025-10-05T22:15Z
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

    # --- PATCH START: resume-day skip balance reset ---
    ...
    # --- PATCH START: maintain start_idx alignment (no checkpoint logic) ---
    try:
        start_idx = 0
    except Exception:
        start_idx = 0
    # --- PATCH END ---
        ...
        for ep in range(epochs):
            ...
            # ✅ flush outcomes every 10 epochs
            if ep > 0 and ep % 10 == 0:
                _wrap_up_posteriors(outcomes, autodb=autodb)
                autodb.commit()
                print(f"[replay] checkpoint flush → {len(outcomes)} outcomes")
                outcomes.clear()

           
            for di, day in enumerate(all_days[start_idx:], start=start_idx+1):
                # --- Count hierarchy ---
...
# ───────────────────────────────────────────────────────────────
                            ...
                            race_name = (brow["race_name"] or "") if brow else ""
                            ...
                            if abs(market_pnls.get(sid, 0.0)) > market_liability_cap:
                                continue

                            market_pnls = get_market_pnl(day, mid)
                            outcomes.append({
                                ...
                                "surface_type": _derive_surface_type(race_name),
                                # ✅ replace pnl with settlement value for this sid
                                "pnl": market_pnls.get(sid, 0.0),
...
# ───────────────────────────────────────────────────────────────
    ...
    if outcomes:
        _wrap_up_posteriors(outcomes, autodb=autodb)
        autodb.commit()
        print(f"[replay] final flush → {len(outcomes)} outcomes")
        outcomes.clear()
...
# 📍 TARGET: engines/replay_runner.py
# 🔎 SEARCH: def _wrap_up_posteriors(outcomes):
# 📆 PATCHED: 2025-10-06
def _wrap_up_posteriors(outcomes, autodb=None):
    """
    Append outcomes to mastery_outcomes_raw only.
    Consolidation into mastery_posteriors is handled later.
    """
    print(f"[replay] Appending {len(outcomes)} outcomes → mastery_outcomes_raw")

    # reuse attached DB connection if provided
    db = autodb if autodb is not None else sqlite3.connect(autoscalp_db())
    ...
    for o in outcomes:
        cur.execute("""
            INSERT INTO mastery_outcomes_raw(
                ...
                weight, priority, created_at
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
            o.get("day"), o.get("mid"), o.get("sid"), o.get("letter"),
            o.get("distance_band"), o.get("fav_rank_bin"),
            o.get("day_of_week"), o.get("surface_type"),
            o.get("class_band"), int(o.get("target_ticks",0)),
            int(o.get("realized_ticks",0)), int(o.get("success",0)),
            int(o.get("stoploss",0)), int(o.get("hedge",0)),
            float(o.get("entry_px",0.0)), float(o.get("pnl",0.0)),
            float(o.get("weight",1.0)), int(o.get("priority",99)),
            now
        ))
    cur.execute("COMMIT")
