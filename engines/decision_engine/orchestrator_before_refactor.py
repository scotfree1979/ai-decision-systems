...
def rulebook_allow(letter: str, pass_n: int, ctx: dict, *, plan: dict) -> tuple[bool, dict]:
    """
    Returns (ok, plan_mutated). Mutates ticks/size if trend confirms. No DB I/O.
    Expects ctx to have: odds, slope_ppm (micro), up_ticks_10s, down_ticks_10s,
    phase, minutes_to_off, optional band/series via callers.
    """
    try:
        # Direction as +1 (drift/LAY->BACK) or -1 (steam/BACK->LAY)
        dir_tag = str(plan.get("direction","LAY->BACK")).upper()
        d = +1 if dir_tag.startswith("LAY") else -1

        # micro/macro slopes (you already compute slope_ppm; macro from context if present)
        s_micro = float(ctx.get("slope_ppm", 0.0) or 0.0)
        s_macro = float(ctx.get("slope_ppm_300", ctx.get("slope_5m", s_micro)) or 0.0)

        # tick pulse
        up10 = int(ctx.get("up_ticks_10s", 0) or 0)
        dn10 = int(ctx.get("down_ticks_10s", 0) or 0)
        pulse = (dn10 - up10) * d  # positive if tape supports our direction

        trend_agree   = (s_micro == 0.0) or ((s_micro > 0 and d == +1) or (s_micro < 0 and d == -1))
        trend_confirm = (s_macro == 0.0) or ((s_macro > 0 and d == +1) or (s_macro < 0 and d == -1))

        # defaults / guards
        odds = float(ctx.get("odds", 0.0) or 0.0)
        spread_ok = True  # if you later add spread, change here
        vol_ok    = abs(s_micro) <= 0.25

        # Light range awareness if provided
        pos_5m    = float(ctx.get("range_pos_5m", 0.5))
        near_bo   = (d == +1 and pos_5m >= 0.85) or (d == -1 and pos_5m <= 0.15)

        # quick lane nuances
        L = letter.upper()
        ticks = int(plan.get("target_ticks", 1)) or 1
        size  = float(plan.get("size", 2.0) or 2.0)

        # PASS 1: exploratory
        if pass_n <= 1:
            ok = spread_ok and vol_ok
            return (ok, {"target_ticks": ticks, "size": size})

        # PASS 2: trend + pulse
        if pass_n == 2:
            if not (trend_agree and pulse >= 2 and spread_ok and vol_ok):
                return (False, plan)
            if trend_confirm:
                ticks = min(3, ticks + 1)
            return (True, {"target_ticks": ticks, "size": size})

        # PASS 3+: stricter, require macro confirm
        if not trend_agree or not trend_confirm or not spread_ok or not vol_ok:
            return (False, plan)

        # lane specific refinements
        if L in ("R","X"):  # breakout or crossover → prefer new extremes / near boundary
            if not (near_bo or pulse >= 2):
                return (False, plan)

        if L == "F":  # steam-fade, exhaustion only
            # counter-move around extreme; allow only small tick target
            if abs(s_micro) < 0.09:
                return (False, plan)
            ticks = 1

        if L == "B":  # BTL must be steam
            if d != -1:
                return (False, plan)

        if L == "G":  # aggressive BTL: stronger pulse
            if pulse < 3:
                return (False, plan)

        # modest reinforcement
        ticks = min(3, ticks + (1 if pulse >= 3 else 0))
        return (True, {"target_ticks": ticks, "size": size})
    except Exception:
        return (True, plan)
...
def _schedule_conn() -> sqlite3.Connection:
    ...
    try:
        ...
        if cnt:
            today = _q_retry(a, "SELECT COUNT(*) FROM markets_schedule WHERE substr(off_at_utc,1,10) IN (date('now','utc'), date('now'))"
            ).fetchone()[0]
            if int(today or 0) > 0:
                return a
    except Exception:
...
def _dbg_sched_counts():
    a = _q_retry(_auto_conn(), "SELECT COUNT(*) FROM markets_schedule WHERE substr(off_at_utc,1,10)=date('now','utc')").fetchone()[0]
    b = _q_retry(_bets_conn(), "SELECT COUNT(*) FROM markets_schedule WHERE substr(off_at_utc,1,10)=date('now','utc')").fetchone()[0]
...
def _load_where_today_utc(bdb) -> list:
    return _q_retry(bdb, f"""
        ...
        SELECT marketId, off_at_utc
        FROM s
        WHERE substr(off_at_utc,1,10) = date('now','utc')
          AND (
                off_dt >= datetime('now','utc')
             OR ((julianday('now','utc') - julianday(off_dt)) * 1440.0) <= 15.0
          )
        ORDER BY off_dt ASC
        """
    ).fetchall()
...
def _in_scope_status(mid: str, *, inplay_window_min: int = 15) -> tuple[bool, str]:
    """
    Canonical in-scope check with a reason:
      returns (True,'ok') or (False,'no_off'|'not_today'|'elapsed_gt_window')
    """
    ...
    now = _utcnow()
    # Ensure we handle timezone-aware vs naive comparisons
    is_today = (
        off.date() == now.date()
    ...
    if elapsed_min <= float(inplay_window_min):
        return (True, "ok")
    return (False, "elapsed_gt_window")


def _is_today_in_scope(mid: str, *, inplay_window_min: int = 15) -> bool:
    ok, _ = _in_scope_status(mid, inplay_window_min=inplay_window_min)
    return ok

# Back-compat alias used elsewhere in the module
_is_today_market = _is_today_in_scope
...
def _scope_pass(run_id: str, ctx: dict, logger=None) -> Optional[int]:
    ...
    # ── tunables
    MAX_PLACEMENTS_PER_TICK = int(os.getenv("AUTOSCALP_MAX_PER_TICK", "99"))  # unlimited-ish by default
    ...
    # seed/maintain the fixed-size window = next 5 markets (near first, then far)
    _seed_scope_window(scope)
    _prune_and_slide_window(scope)

    # dashboard print (kept)
    ...
    # priority key: TTO ≤ 20 first, then smaller TTO
    tto_map = {m: t for (m, t) in (scope.get("pre_near", []) + scope.get("pre_far", []))}
    def _prio_key(m):
    ...
    # ── minute-throttle state (reset once per minute), letter-aware
    try:
    ...
    attempted = getattr(_scope_pass, "_ATTEMPTED_MINUTE", set())
    last_wall = getattr(_scope_pass, "_ATTEMPTED_WALL", None)
    if last_wall != wall_min:
        attempted = set()
        setattr(_scope_pass, "_ATTEMPTED_MINUTE", attempted)
        setattr(_scope_pass, "_ATTEMPTED_WALL", wall_min)

    # registry (safe if missing)
    try:
    ...
    # ensure families default to enabled if mask missing
    ...
    # iterate all in-scope markets (bounded), then their active runners
    for mid in ordered[:MAX_MARKETS]:
        ok_scope, why_scope = _in_scope_status(mid, inplay_window_min=15)
        if not ok_scope:
            log(f"[SCOPE] skip stale/non-today mid={mid} reason={why_scope}")
            continue

        # recompute minutes-to-off + phase hints for this market
        try:
        ...
        try:
            ...
            phase = "IN_PLAY" if raw == "OFF" else ("PRE" if raw == "PRE" else ("IN_PLAY" if float(mto_fix) <= 0.0 else "PRE"))
        except Exception:
            phase = "IN_PLAY" if float(mto_fix) <= 0.0 else "PRE"
        ...
        for sid, odds in cands:
            ...
            if win_fix: ctx["tto_window"] = win_fix
            ctx["phase"]          = phase
            ...
            if keyA in _ATTEMPTED:
                ...
                if picked >= MAX_RUNNERS:
                    break
                continue
            _ATTEMPTED.add(keyA); setattr(_scope_pass, "_ATTEMPTED_MINUTE", attempted)
            ...
            # ---------- Registry families (respect PRE/IP) ----------
            ...
            for (fname, ffn) in (ORDER_LIST or []):
                ...
                # per-letter minute throttle
                keyF = (mid, sid, letter, wall_min)
                if keyF in _ATTEMPTED:
                    continue
                _ATTEMPTED.add(keyF); setattr(_scope_pass, "_ATTEMPTED_MINUTE", attempted)

                if _already_open_pass(mid, sid, pass_tag, mode=ctx.get("source","LIVE")):
                    continue
                ctx["pass_tag"] = pass_tag

                # Try primary + alternates
                placed_here = None
                try:
                    plan2 = ffn(ctx)
                    if plan2 and plan2.get("enter"):
            ...
            # ---------- Legacy S-scout as last PRE fallback ----------
            if phase == "PRE" and 0.0 < float(mto_fix) <= 60.0 and 1.5 <= float(odds) <= 8.0:
                ...
                if keyS in _ATTEMPTED:
                    ...
                    if picked >= MAX_RUNNERS:
                        break
                    continue
                _ATTEMPTED.add(keyS); setattr(_scope_pass, "_ATTEMPTED_MINUTE", attempted)
    ...
    # finished the whole sweep; still rotate the starting point
    _advance_scope_cursor(1, n)
    return placed_first
...
def _auto_conn(retries: int = 6, delay_s: float = 0.08) -> _sqlite.Connection:
    last = None
    for i in range(max(1, retries)):
        try:
            con = _sqlite.connect(autoscalp_db(), timeout=15, isolation_level=None, check_same_thread=False)
            con.row_factory = _sqlite.Row
            try:
                _q_retry(con, "PRAGMA busy_timeout=8000")
...
def _q_retry(con: sqlite3.Connection, sql: str, params: tuple = (),
             tries: int = 6, delay_s: float = 0.08) -> sqlite3.Cursor:
    """
    Execute SQL with retry on 'locked'/'unable to open database file'.
    """
    import time as _t
    last = None
...
def place_strategy_instruction(run_id: str, marketId: str, selectionId: str,
    ...
    try:
        letter = (source or "")[:1].upper() if source else None
        can_open, why = _can_open_scalp(
            marketId, selectionId, max_per_runner=3,
            run_id=run_id, family_letter=letter
        )
        if not can_open:
            return False
...
# --- add this helper near _auto_reconcile_worker or alongside other loops ----
def _start_settlement_loop(period_s: int = 300, soft_limit_mb: int = 1024):
    """
    Daemon loop: call settlements.py fetch (last 2 days) then reconcile, every period_s.
    If current RSS > soft_limit_mb, skip this cycle (no data loss; try next).
    """
    import threading, subprocess, sys, os, time, gc, platform
    try:
        import psutil
    ...
    from engines.config_paths import repo_root
    script = os.path.join(repo_root(), "engines", "live", "settlements.py")
    ...
    def _rss_mb():
        if _proc is not None:
            try:
                ...
                pass
        # last resort: no psutil → pretend OK
        return 0.0

    def _runner():
        while True:
            # one-shot “skip this cycle” flag from GUI MemoryGuard
            try:
                if os.path.exists(os.path.join(base, ".settlement_skip_once")):
                    os.remove(os.path.join(base, ".settlement_skip_once"))
                    time.sleep(max(60, int(period_s)))  # skip this period
                    continue
            except Exception:
                pass

            try:
                # Soft guard: if we are memory-tight, skip this cycle
                if _rss_mb() > float(soft_limit_mb):
                    # small GC to help the GUI keep breathing
                    try: gc.collect()
                    except Exception: pass
                else:
                    # pre-GC before spawning subprocesses
                    try: gc.collect()
                    except Exception: pass

                    subprocess.run(
                        [sys.executable, script, "fetch", "--since-days", "2"],
                        check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, close_fds=True
                    )
                    subprocess.run(
                        [sys.executable, script, "reconcile"],
                        check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, close_fds=True
                    )

                    # post-GC after they exit
                    try: gc.collect()
                    except Exception: pass
            except Exception:
                pass
            time.sleep(max(60, int(period_s)))

    # start only once
    try:
        if getattr(_start_settlement_loop, "_t", None) and _start_settlement_loop._t.is_alive():
            return
    except Exception:
        pass
    t = threading.Thread(target=_runner, name="SettlementLoop", daemon=True)
...
def _ensure_ledger_tables():
    ...
    if not _q_retry(db, "SELECT 1 FROM mastery_state WHERE id=1").fetchone():
        _q_retry(db, "INSERT INTO mastery_state(id,progress,updated_at) VALUES (1,0,datetime('now','utc'))")
    ...
    def _cols(tab: str) -> set[str]:
        return {r["name"] for r in _q_retry(db, f"PRAGMA table_info({tab})")}
...
def _end_run_ledger(run_id: str, amount: float, run_day: int, month_index: int):
    ...
    _q_retry(db, "UPDATE sim_runs SET ended_at=datetime('now','utc') WHERE run_id=?", (run_id,))
...
def _sum_pnl_trades_test() -> float:
    ...
    if not exists:
        return 0.0
    has_src = any(r[1] == 'source' for r in _q_retry(db, "PRAGMA table_info('pnl_trades')"))
...
def _compute_minutes_to_off(market_id: str, source: str = "LIVE") -> tuple[float|None, str]:
    """
    Use autoscalp_gui.markets_schedule to compute (minutes_to_off, window_label).
    """
    con = _auto_conn()
    try:
        row = _exec_retry(con,
            "SELECT off_at_utc FROM markets_schedule WHERE marketId=? LIMIT 1",
            (str(market_id),)
        ).fetchone()
        if not (row and row["off_at_utc"]):
            return None, ""
        # off_at_utc is ISO Z
        from datetime import datetime, timezone
        off = datetime.fromisoformat(str(row["off_at_utc"]).replace("Z","+00:00")).astimezone(timezone.utc)
        mto = (off - datetime.now(timezone.utc)).total_seconds() / 60.0
        # window label (same as your builder)
...
def _build_scope(now_utc: Optional[datetime] = None,
    ...
    try:
        bdb = _schedule_conn(); bdb.row_factory = sqlite3.Row
    ...
    def _load_where(day_sql: str) -> list:
        try:
            return _q_retry(bdb, "SELECT marketId, off_at_utc FROM markets_schedule "
    ...
    rows = _load_where_today_utc(bdb)
    if not rows:
        # if AUTO had none, try BETS_DB explicitly
        try:
            b2 = connect_db(ro=True); b2.row_factory = sqlite3.Row
            rows = _q_retry(b2, "SELECT marketId, off_at_utc FROM markets_schedule "
                "WHERE substr(off_at_utc,1,10)=date('now','utc') ORDER BY off_at_utc ASC"
            ).fetchall()
            b2.close()
        except Exception:
            rows = []
    ...
    # 🔧 NEW: hard-filter schedule by canonical in-scope test
    rows = [r for r in rows if _is_today_in_scope(str(r["marketId"]))]

    # If no future races remain today, do NOT fall back to tomorrow in LIVE:
    if not rows:
        if _current_source().upper() == "TEST":
    ...
    if not out["pre_far"] and not out["pre_near"] and not out["in_play"]:
        if _current_source().upper() == "TEST":
            try:
                mids = _q_retry(con, "SELECT DISTINCT marketId FROM inbound_oc_cache "
                    "WHERE datetime(COALESCE(last_sync_ts,'')) >= datetime('now','-24 hours','utc')"
                ).fetchall()
                mids = [str(r[0]) for r in mids or []]
                # In TEST, still keep only “today + future or ≤15m after OFF”
                mids = [m for m in mids if _is_today_in_scope(m)]
                out["pre_far"] = [(m, float('inf')) for m in mids][:max_markets]
            except Exception:
                pass
    else:
        # LIVE: no inbound fallback — keep scope empty if schedule is empty
        pass



    try:
        if os.getenv("AUTOSCALP_DEBUG_SCOPE_COUNTS", "0") == "1":
...
def _tbl_exists(conn: sqlite3.Connection, name: str) -> bool:
    row = _q_retry(conn, "SELECT 1 FROM sqlite_master WHERE type='table' AND name=?",
        (name,)
    ).fetchone()
    return bool(row)
...
def _has_col(conn: sqlite3.Connection, table: str, col: str) -> bool:
    ...
    """
    try:
        cur = _q_retry(conn, f"PRAGMA table_info({table})")
        for r in cur:
...
def reset_pnl_ledger(logger=None) -> None:
    ...
    if _q_retry(db, "SELECT 1 FROM sqlite_master WHERE type='table' AND name='pnl_daily'").fetchone():
        has_src = any(r[1]=='source' for r in _q_retry(db, "PRAGMA table_info('pnl_daily')"))
...
def _fallback_plan_for_letter(letter: str, ctx: dict) -> dict:
    """
    Unified fallback plan for any family letter:
      - Direction: steam → BACK->LAY, drift → LAY->BACK, else odds≥4 → LAY->BACK, else BACK->LAY
      - Ticks: 2 if |slope_ppm| >= 0.08 else 1
      - Size: dynamic per-letter (mastery); if unavailable, size_cap or £2
    """
    try:
    ...
    # Dynamic size identical everywhere (matches S)
    try:
        from engines.mastery.mastery_policy import size_for_letter as _size_for_letter
        size = float(_size_for_letter(ctx, str(letter or "S")))
    except Exception:
    ...
    return {
        "enter": True,
        "direction": direction,
        "target_ticks": int(max(1, ticks)),
        "size": max(2.0, float(size)),
        "why": f"{letter or '?'}-fallback dir={direction} slope={slope:.3f} odds={odds:.2f}"
    }
    ...
    try:
        ...
        for r in rows:
            ...
            if status == "EXECUTION_COMPLETE":
                # mark child matched
                _q_retry(con, "UPDATE orders SET entry_status='matched', closed_at=datetime('now','utc') WHERE id=?", (int(r["id"]),))
                con.commit()
...
def _ensure_decisions(con: sqlite3.Connection) -> None:
    ...
    cols = [r["name"] for r in _q_retry(con, "PRAGMA table_info(decisions)")]
...
def _mark_last_child_placed(parent_id: int) -> None:
    ...
    if row:
        _q_retry(con, "UPDATE orders SET entry_status='placed' WHERE id=?", (int(row["id"]),))
        con.commit()
...
def _log_once_per_minute(mid: str, sid: str, level: str, msg: str) -> None:
    """
    Throttle noisy logs per (mid,sid) per minute. Avoid flooding on DNS blips.
    """
    try:
        now_min = int(_utcnow().timestamp() // 60)
        key = (str(mid), str(sid))
        last = _SEEN_FETCH_ERR_MINUTE.get(key, -1)
        if last == now_min:
            return
        _SEEN_FETCH_ERR_MINUTE[key] = now_min
        _log_event(level, "odds_fetch", f"{mid}/{sid} {msg[:200]}")
    except Exception:
        pass
...
def _gate_bump(reason: str) -> None:
    """Count a gate reason once into the current wall-minute bucket."""
    global _GATE_BUCKET_MIN, _GATE_COUNTS
    try:
        wall = int(_utcnow().timestamp() // 60)
    except Exception:
        from time import time as _t
        wall = int(_t() // 60)

    if _GATE_BUCKET_MIN != wall:
        _GATE_BUCKET_MIN = wall
        _GATE_COUNTS.clear()

    key = (reason or "unknown").strip().lower()
    _GATE_COUNTS[key] = _GATE_COUNTS.get(key, 0) + 1

def _gate_snapshot() -> str:
    """One compact line with sorted counts for the current minute bucket."""
    if not _GATE_COUNTS:
        return "gates: {}"
    parts = [f"{k}={v}" for k, v in sorted(_GATE_COUNTS.items(),
...
def _latest_price(marketId: Optional[str], selectionId: Optional[str]) -> Tuple[Optional[float], Optional[list]]:
    ...
    # NEW: if this looks like a reserve/unknown runner in today's feed, don't chase prices
    if not _runner_known(mid, sid):
        # quietly: no odds available for reserves/withdrawn
        return (None, None)
    ...
    try:
        ...
        if _tbl_exists(con, "inbound_oc_cache"):
            row = _q_retry(con, "SELECT oc1, oc1_band_json FROM inbound_oc_cache "
    ...
    try:
        bdb = connect_db(ro=True); bdb.row_factory = sqlite3.Row
        r = _q_retry(bdb, "SELECT odd FROM oc_series WHERE marketId=? AND selectionId=? "
    ...
    try:
        if os.getenv("AUTOSCALP_ENABLE_DIRECT_ODDS", "1") == "1":
...
def _active_candidates_for_market(market_id: str, max_runners: int = 4) -> list[tuple[str, float]]:
    """
    Return up to max_runners [(selectionId, odds)] that are:
      • in-scope today,
      • within the active odds band (1.5..8.0),
      • classified 'active' by _runner_activity,
    with a per-market round-robin so we rotate through all eligible runners.

    Source order (fast → slow):
      1) inbound_oc_cache (latest oc1 per runner)
      2) oc_series (latest odd per runner)
      3) runners + _latest_price(mid,sid) (series/inbound/direct fallback)
    """
    mid = str(market_id)

    # HARD GUARD: never propose candidates for stale markets
    if not _is_today_in_scope(mid):
        return []

    # --- Build latest-odds map with retries ---------------------------------
    odds: dict[str, float] = {}
    seen: set[str] = set()
    ...
    try:
        # 1) inbound_oc_cache latest per selectionId
        try:
            if _q_retry(con, "SELECT 1 FROM sqlite_master WHERE type='table' AND name='inbound_oc_cache'"
            ).fetchone():
                rows = _q_retry(
                    con,
                    """
                    SELECT t.selectionId, t.oc1
                ...
                for r in rows or []:
                    ...
                    oc1 = r["oc1"]
                    if oc1 is not None and sid not in odds:
                        try:
                            odds[sid] = float(oc1)
                        except Exception:
                            pass
        ...
        # 2) oc_series latest per selectionId (fill gaps only)
        if len(odds) < max_runners:
            try:
                if _q_retry(con, "SELECT 1 FROM sqlite_master WHERE type='table' AND name='oc_series'"
                ).fetchone():
                    rows2 = _q_retry(
                        con,
                        """
                        SELECT s.selectionId, s.odd
                        FROM oc_series s
                        JOIN (
                            SELECT selectionId, MAX(datetime(snapshot_ts)) AS mx
                            FROM oc_series
                            WHERE marketId=?
                            GROUP BY selectionId
                        ) x ON x.selectionId=s.selectionId AND x.mx=datetime(s.snapshot_ts)
                        WHERE s.marketId=?
                        """,
                        (mid, mid)
                    ).fetchall()
                    for r in rows2 or []:
                        sid = str(r["selectionId"])
                        o   = r["odd"]
                        if sid not in odds and o is not None:
                            try:
                                odds[sid] = float(o)
                            except Exception:
                                pass
        ...
        # 3) runners table + _latest_price() fallback for any remaining sids
        if len(odds) < max_runners:
            try:
                if _q_retry(con, "SELECT 1 FROM sqlite_master WHERE type='table' AND name='runners'"
                ).fetchone():
                    rnames = _q_retry(
                        con,
    ...
    # Fill odds via fallback price path for any sids we saw but still lack a price
    for rr in (rnames or []):
        sid = str(rr["selectionId"])
        if sid in odds:
            continue
        try:
            price, _band = _latest_price(mid, sid)
            if price is not None:
                odds[sid] = float(price)
        except Exception:
            pass
    ...
    def _is_active_pair(sid: str, o: float) -> bool:
        ...
        try:
            ...
            # Be conservative if classifier misbehaves
            return False

    candidates = [(sid, px) for sid, px in odds.items() if _is_active_pair(sid, px)]

    # If nothing survived, bail early
    if not candidates:
        return []

    # Optional: snapshot activity for passing candidates (best-effort)
    for sid, px in candidates:
        try:
            _snapshot_runner_activity(mid, sid, float(px), None)
        except Exception:
            pass

    # --- Round-robin selection so we don't fixate on one runner --------------
    # Keep a stable, per-market rotation cursor in module globals.
    try:
        rr_key = "_RR_CURSOR"
        order_key = "_RR_ORDER"
        ...
        # Build/refresh the stable order for this market from the current candidates.
        # Sort by odds asc (fav → bigger prices), then by selectionId for stability.
        cur_ids = [sid for (sid, _px) in sorted(candidates, key=lambda t: (float(t[1]), str(t[0])))]

        # If the universe changed (e.g., new/removed actives), refresh order but
        # keep the cursor modulo new length to avoid resets.
        ...
        # On any rotation bookkeeping issue, just fall back to a simple sorted slice
        sorted_pairs = sorted(candidates, key=lambda t: (float(t[1]), str(t[0])))
        return sorted_pairs[:max_runners]

    # ───────────────────────────────────────────────────────────────────
    # FINAL FALLBACK: top up candidates with direct odds if cache is thin
    # ───────────────────────────────────────────────────────────────────
    try:
        MIN_WANT = min(8, max_runners)
        if len(cands) < MIN_WANT:
            # 1) get a runner list fast (any table will do)
            extra_sids = []
            try:
                con = _auto_conn()
                rows = _q_retry(con, "SELECT selectionId FROM runners WHERE marketId=? LIMIT 24",
                                (market_id,)).fetchall()
                extra_sids = [str(r["selectionId"]) for r in rows]
                con.close()
            except Exception:
                extra_sids = []

            # 2) fetch live price per missing sid (cap to avoid hammering)
            try:
                from engines.upgrade_import_patch import get_session_token  # type: ignore
                token = (get_session_token() or "").strip()
            except Exception:
                token = os.environ.get("BETFAIR_SESSION", "") or ""

            seen = {sid for (sid, _px) in cands}
            budget = max(0, MIN_WANT - len(cands))
            asked = 0
            for sid in extra_sids:
                if asked >= 12 or budget <= 0:
                    break
                if sid in seen:
                    continue
                price = None
                try:
                    from engines.utils.api_tools import fetch_live_odds
                    odds = fetch_live_odds(token, market_id, sid) or {}
                    price = odds.get("lay") or odds.get("back")
                except Exception:
                    price = None
                if isinstance(price, (int, float)) and price >= 1.01:
                    cands.append((sid, float(price)))
                    seen.add(sid)
                    budget -= 1
                asked += 1
    except Exception:
...
# Some environments lost this helper during earlier edits; define it if missing.
    ...
    def _ensure_orders_link_col(con: sqlite3.Connection) -> str:
        ...
        if "parent_id" in cols:
            return "parent_id"
        _q_retry(con, "ALTER TABLE orders ADD COLUMN hedge_of INTEGER")
        con.commit()
        return "hedge_of"
...
def _queue_order(run_id: str, side: str, odds: float, stake: float,
    ...
    _q_retry(con, sql, base_vals)
    oid = int(_q_retry(con, "SELECT last_insert_rowid()").fetchone()[0])
    con.commit()
    return oid
...
def _place_decision(ctx: Dict, plan: Dict, marketId: Optional[str], selectionId: Optional[str],
    ...
    has_run = any(r["name"] == "run_id" for r in _q_retry(con, "PRAGMA table_info(decisions)"))
    if has_run:
        _q_retry(con, "INSERT INTO decisions (decided_at, marketId, selectionId, signal_type, confidence, meta_json, order_id, run_id) "
            "VALUES (datetime('now','utc'), ?, ?, ?, ?, ?, ?, ?)",
            (*vals, run_id)
        )
    else:
        _q_retry(con, "INSERT INTO decisions (decided_at, marketId, selectionId, signal_type, confidence, meta_json, order_id) "
            "VALUES (datetime('now','utc'), ?, ?, ?, ?, ?, ?)",
            vals
        )
    con.commit()

def _place_companion_hedge(parent_id: int,
                           direction: str,
                           entry_odds: float,
                           parent_stake: float,
                           target_ticks: int,
    ...
    IMPORTANT: child inherits the *parent's* strategy source (e.g. I6/X2/etc.), not "LIVE".
    """
    # Compute target odds & hedge stake using green-up formulas
    n_ticks = abs(_si(target_ticks, 1))
    if str(direction).startswith("LAY"):            # LAY->BACK (need drift ↑)
        target_odds = _odds_plus_ticks(float(entry_odds), n_ticks)
        hedge_side  = "BACK"
    else:                                            # BACK->LAY (need steam ↓)
        target_odds = _odds_plus_ticks(float(entry_odds), -n_ticks)
        hedge_side  = "LAY"
    ...
    try:
        ...
        # ── inherit parent source (e.g., I6 / X2 / R1…) ──────────────────────
        parent_src = None
        try:
            r = _q_retry(con, "SELECT source FROM orders WHERE id=?", (int(parent_id),)).fetchone()
            if r and r["source"]:
                parent_src = str(r["source"])
        except Exception:
            parent_src = None

        # If parent had no source, fall back to mode tag (LIVE/TEST/LEARNING)
        src = parent_src or (mode_override or _current_source().upper())

        base_cols = ["mode","side","entry_odds","entry_stake","entry_status","marketId","selectionId"]
        base_vals = [ (mode_override or _current_source().upper()),
                      hedge_side, float(target_odds), float(hedge_stake), "queued",
                      str(marketId), str(selectionId) ]

        # link to parent
        base_cols.append(link_col)
        base_vals.append(int(parent_id))

        if has_run:
            base_cols.append("run_id")
            base_vals.append(run_id)
        if has_src:
            base_cols.append("source")
            base_vals.append(src)                       # ← child inherits parent's source
        if has_role:
            base_cols.append("role")
            base_vals.append("CHILD")

        sql = f"INSERT INTO orders ({', '.join(base_cols)}, opened_at) VALUES ({', '.join(['?']*len(base_cols))}, datetime('now','utc'))"
        _q_retry(con, sql, base_vals)
        child_id = int(_q_retry(con, "SELECT last_insert_rowid()").fetchone()[0])
        con.commit()
        return child_id
    except Exception:
        return None
...
def _set_order_status(order_id: int, status: str, net_pl: float | None = None) -> None:
    ...
    if status in ("matched", "cancelled"):
        if net_pl is None:
            _q_retry(con, "UPDATE orders SET entry_status=?, closed_at=datetime('now','utc') WHERE id=?",
                (status, order_id),
            )
        else:
            _q_retry(con, f"UPDATE orders SET entry_status=?, closed_at=datetime('now','utc'), "
                f"{pnl_col}=COALESCE({pnl_col},0.0)+? WHERE id=?",
                (status, float(net_pl), order_id),
            )
    else:
        _q_retry(con, "UPDATE orders SET entry_status=? WHERE id=?", (status, order_id))
...
def start_live_loop(run_id: str, interval: float = 0.8, logger=None, **kwargs):
    ...
    # Optional: background re-hedge loop (harmless if fails)
    try:
    ...
    while True:
        try:
            ...
            try:
                ...
                if mid:
                    mto = _compute_minutes_to_off(str(mid), source=_current_source())
                    con = _auto_conn()
...
def _already_open_pass(market_id: str, selection_id: str, pass_tag: str, *, mode: str = "LIVE") -> bool:
    """
    True if a PARENT with notes=pass_tag exists TODAY for this runner in an IN-SCOPE market,
    where the hedge child is NOT matched yet.
    """
    import sqlite3
    con = _auto_conn()
    con.row_factory = sqlite3.Row
    ...
    parent_pred = "p.role='PARENT'" if has("role") else (f"(p.{link} IS NULL OR p.{link}='')" if link else "1=1")
    ...
    where = [
        ...
        "p.marketId=?",
        "p.selectionId=?",
        "date(p.opened_at)=date('now','utc')",
        # in-scope today (future off OR OFF within window=15m)
        """
        EXISTS (
          SELECT 1
          FROM markets_schedule s
          WHERE s.marketId = p.marketId
            AND substr(s.off_at_utc,1,10)=date('now','utc')
            AND (
                  datetime(replace(replace(s.off_at_utc,'T',' '),'Z','')) >= datetime('now','utc')
               OR ((julianday('now','utc') - julianday(datetime(replace(replace(s.off_at_utc,'T',' '),'Z','')))) * 1440.0) <= 15.0
            )
        )
        """
    ]
    ...
    row = _q_retry(con, sql, tuple(args)).fetchone()
...
def _family_lanes_pass(run_id: str, ctx: dict, logger=None) -> Optional[int]:
    ...
    def _fallback_plan_for_letter(letter: str, ctx: dict) -> dict:
        """
        Minimal, safe plan if the registry has no strategy function for this letter.
        PRE families (B/G/X/R/F/L): use signal -> direction, ticks 1-2, size=ctx.size_cap.
        IP family (I): same, but phase already IN_PLAY in our caller.
        """
        # direction from tape slope, else odds threshold, same logic as A/S
        try:
            direction = _direction_from_signal(ctx, fallback_odds=float(ctx.get("odds") or 0.0))
        except Exception:
            direction = "LAY->BACK"
        try:
        ...
        return {
            ...
            "target_ticks": max(1, int(ticks)),
            "size": max(2.0, min(size_cap, 5.0)),
            "why": f"fallback_{letter} dir={direction} sppm={sppm:.3f}",
        }
...
def _apply_rulebook(letter: str, preview_code: str, ctx: dict, plan: dict):
    """
    Uniform rulebook application:
      - Derives pass_n from preview_code (e.g., 'A12' -> 12)
      - Calls rulebook_allow(...)
      - Applies any 'adj' (tighten size/target_ticks) into plan
      - Returns (skip_this: bool, plan: dict)
    """
    # pass number
    try:
        if len(preview_code) > 1:
            digits = ''.join(ch for ch in preview_code[1:] if ch.isdigit())
            pass_n = int(digits) if digits else 1
        else:
            pass_n = 1
    except Exception:
        pass_n = 1
    ...
    adj = None
    try:
        ok_rb, adj = rulebook_allow(letter, pass_n, ctx, plan=plan)
        # For passes ≥ 2, rulebook may veto the attempt
        if pass_n >= 2 and not ok_rb:
            skip_this = True
    except Exception:
        skip_this = False
        adj = None

    # Apply adjustments (ticks/size) if provided
    if isinstance(adj, dict) and adj and isinstance(plan, dict):
        for k in ("target_ticks", "size"):
            if k in adj:
                plan[k] = adj[k]

    return skip_this, plan
...
def decide_once(run_id: str, source_override: str | None = None, logger=None) -> Optional[int]:
    ...
    # --- per-tick debug counters (compact heartbeat) -------------------------
    _dbg = {
        "cand_seen": 0,         # how many (mid,sid) candidates we evaluated
        "placed": 0,            # how many we actually placed
        # gate buckets:
        "no_price": 0,          # missing/latest price
        "out_of_band": 0,       # odds not in 1.5..8.0 (A/S lanes)
        "not_in_scope": 0,      # market failed _is_today_in_scope
        "inactive": 0,          # _runner_activity != active
        "cap": 0,               # per-runner cap reached
        "rule": 0,              # rulebook veto
        "veto": 0,              # generic skip_this/other veto
    }

    # ── DEC heartbeat (once per minute; compact) ─────────────────────────────
    try:
        wall_min = int(_utcnow().timestamp() // 60)
    except Exception:
        from time import time as _t; wall_min = int(_t() // 60)

    last_hb = getattr(decide_once, "_HB_WALL", None)
    if last_hb != wall_min:
        # lightweight scope probe (safe if builder fails)
        try:
            _sc = _build_scope(now_utc=_utcnow(), inplay_window_min=15)
            pre_far_n  = len(_sc.get("pre_far",  []))
            pre_near_n = len(_sc.get("pre_near", []))
            in_play_n  = len(_sc.get("in_play",  []))
        except Exception:
            pre_far_n = pre_near_n = in_play_n = -1

        src = (ctx.get("source") or _current_source()).upper()
        midtag = str(market_id) if market_id else "-"
        sidtag = str(selection_id) if selection_id else "-"

        print(f"[DEC HB] src={src} mid={midtag} sid={sidtag} "
              f"scope pf={pre_far_n} pn={pre_near_n} ip={in_play_n} "
              f"tto={ctx.get('tto_minutes')} odds={ctx.get('odds')}")
        setattr(decide_once, "_HB_WALL", wall_min)


    def _hit(key: str):
        _dbg[key] = _dbg.get(key, 0) + 1
    ...
    # =====================================================================
    # 1) Place from plan _place_from_plan
    # =====================================================================
    ...
    def _place_from_plan(_name: str, _plan_like, _ctx: dict) -> Optional[int]:
        ...
        # tiny logger shim: use the decide_once logger, not self
        _log = logger
        ...
        # ⬇️ size comes from plan; still apply L1 clamp below
        # OLD:
        # size      = float(plan.get("size") or 2.0)

        # NEW: default to mastery’s per-letter sizing when plan.size not provided
        try:
            from engines.mastery.mastery_policy import size_for_letter as _size_for_letter
        except Exception:
            _size_for_letter = lambda _ctx, _letter: float(_ctx.get("size_cap", 2.0) or 2.0)

        size = float(plan.get("size") or _size_for_letter(_ctx, letter))
        ...
        if not px or px <= 0.0:
            ...
            _dbg["cand_seen"] += 1
            try:
                px = f_or(ctx.get("current_odds"), 0.0)
                if px <= 0.0:
                    return None  # or continue, depending on the lane
            except Exception:
                return None

    

        # ---- HARD GATE: active odds + scope everywhere -----------------------
        try:
            # odds band (active book)
            if not (1.5 <= float(px) <= 8.0):
                _hit("out_of_band")
                _dbg["cand_seen"] += 1
                return None

            # market must be in-scope (today + future or ≤15m after off)
            if not _is_today_in_scope(mid):
                _hit("not_in_scope")
                _dbg["cand_seen"] += 1
                return None

            # runner must be classified 'active'
            if _runner_activity(str(mid), str(sid)) != "active":
                _hit("inactive")
                _dbg["cand_seen"] += 1
                return None
        except Exception:
            # fail-closed on any gate evaluation issue
            return None
        ...
        if l1f > 0.0:
            ...
            size = min(size, liq_cap)   # ⬅️ clamp the plan size, don’t overwrite elsewhere


        # ---- per-market family code (A1/S2/X3/…) & per-letter cap ------------
        code = _next_pair_tag(letter, mid)

        # Rulebook gate (no loop here → return early on veto)
        skip_this, plan = _apply_rulebook(letter, code, _ctx, plan)
        if skip_this:
            _hit("rule")
            _dbg["cand_seen"] += 1
            return None
        ...
        try:
            con = _auto_conn()
            _q_retry(con, "UPDATE orders SET source=?, notes=? WHERE id=?",
        ...
        # --- compute child target & hedge stake (green-up) --------------------
        t = max(1, int(ticks))
        tgt_odds = _odds_plus_ticks(px, +t) if side == "LAY" else _odds_plus_ticks(px, -t)
        hedge_stake = float(size) * float(px) / max(1e-9, float(tgt_odds))


        # ---- LIVE route (parent now, child with correct stake) ---------------
        if (_ctx.get("source") or "LIVE").upper() == "LIVE":
            try:
                # Prefer a router that accepts hedge_stake explicitly
                from engines.live.live_router import place_parent_and_hedge as _router_ph
                bet_id, cref = _router_ph(
                    market_id=mid, selection_id=sid,
                    side=side, entry_odds=float(px),
                    stake=float(size),
                    hedge_ticks=int(t), hedge_stake=float(hedge_stake),
                    source=code,
                    parent_persistence="LAPSE",
                    child_persistence=("LAPSE" if letter in ("A","S","B","F","L","X","O") else "PERSIST"),
                )
            except Exception:
                # Fallback: legacy router (child computes stake internally)
                try:
                    from engines.live.live_router import place_parent_and_hedge as _router_ph
                    bet_id, cref = _router_ph(
                        market_id=mid, selection_id=sid,
    ...
    # --- Mastery plan shim used in ALL lanes ---
    def _plan_from_mastery(_ctx: dict) -> dict:
        """
        Ask mastery for a plan (always returns dict or {}), fail-quiet.
        """
        try:
            from engines.mastery.mastery_policy import propose_trade as _mpt
            plan = _mpt(_ctx) or {}
            # Normalize: mastery's plan always uses 'direction', 'target_ticks', 'size'
            if "enter" not in plan:
                plan["enter"] = bool(plan.get("direction")) and bool(plan.get("target_ticks"))
            return plan
        except Exception:
            return {}
    ...
    # NEW: run the family lanes (B,G,X,R,F,L, then I) with scope + Active filter
    placed_id = _family_lanes_pass(run_id, ctx, logger=logger)
    if placed_id:
        return placed_id
    ...
    # =====================================================================
    # PRE/IN-PLAY family lanes (run BEFORE A). Same plan path as S/A.
    # - PRE lanes: no minute time-gate (still PRE-only); Active runners only.
    # - IP  lanes: IN-PLAY only; Active runners only.
    # Stop after first successful placement.
    # =====================================================================
    scope = _build_scope(now_utc=_utcnow(), inplay_window_min=15)

    # Choose representative strategy names that map to letters via STRAT_CODE
    PRE_LANES = [
        "BTL_SCOUT",        # B
        "BTL_AGGR",         # G
        "S4_CROSSOVER",     # X
    ...
    # Helper to run a lane list over scope and place once
    def _run_lane_list(_names: list[str]) -> Optional[int]:
        MAX_MARKETS = 6
        MAX_RUNNERS = 2
        ordered_markets = list(scope.get("pre_far", [])) + list(scope.get("pre_near", [])) + list(scope.get("in_play", []))

        for mid, _t in ordered_markets[:MAX_MARKETS]:
            # phase for this market (prefer Betfair)
            try:
                mto_fix, win_fix = _compute_minutes_to_off(mid, source=ctx.get("source", "LIVE"))
            except Exception:
                mto_fix, win_fix = (9999.0, "")
            ...
            # Active candidates only (1.5..8.0)
            cands = _active_candidates_for_market(mid, max_runners=8)
            if not cands:
                continue

            for fname in _names:
                # Map name -> family letter
                try:
                    letter = STRAT_CODE.get(fname.upper(), "S")
                except Exception:
                    letter = "S"

                # Phase gate
                if fname in PRE_LANES and phase != "PRE":
                    continue
                if fname in IP_LANES and phase != "IN_PLAY":
                    continue

                picked = 0
                for sid, odds in cands:
                    # per-minute throttle per (market, runner, lane)
                    try:
                    ...
                    _ATTEMPTED = getattr(decide_once, "_ATTEMPTED_MINUTE", set())
                    keyL = (mid, sid, fname, wall_min_bucket)
                    if keyL in _ATTEMPTED:
                        continue
                    _ATTEMPTED.add(keyL)
                    setattr(decide_once, "_ATTEMPTED_MINUTE", _ATTEMPTED)
                    ...
                    # Plan via mastery first, fallback legacy
                    try:
                        plan = _plan_from_mastery(ctx)
                    except Exception:
                        plan = None
                    if not plan or not plan.get("enter"):
                        plan = _legacy_scout_plan(ctx)

                    # ---------- per-pass rulebook for families ----------
                    preview_code = _next_pair_tag(letter, mid)
                    skip_this, plan = _apply_rulebook(letter, preview_code, ctx, plan)
                    if skip_this:
                        picked += 1
                    ...
                    # (keep plan as returned; do NOT fall back here yet)

           
                    # Per-letter cap (3 per runner per letter)
                    ok_cap, _why = _can_open_scalp(mid, sid, max_per_runner=3, run_id=run_id, family_letter=letter)
                    if not ok_cap and (ctx.get("source","LIVE") != "TEST"):
                        _hit("cap")
                        _dbg["cand_seen"] += 1
                        picked += 1
    ...
    # Run PRE families, then IP families; stop after first placement
    placed_lane = _run_lane_list(PRE_LANES)
    if placed_lane:
        return placed_lane
    placed_lane = _run_lane_list(IP_LANES)
    if placed_lane:
        return placed_lane
    ...
    # =====================================================================
    # A-lane (ALWAYS_ON): same plan logic as S, but NO time gate (still PRE-only)
    # =====================================================================
    scope = _build_scope(now_utc=_utcnow(), inplay_window_min=15)
    ...
    for mid, _tto in ordered_markets[:MAX_MARKETS_A]:
        # recompute minutes-to-off and phase for this market
        try:
            mto_fix, win_fix = _compute_minutes_to_off(mid, source=ctx.get("source","LIVE"))
        except Exception:
            mto_fix, win_fix = (9999.0, "")
        ...
        # A is PRE-only (but no TTO min/max gate)
        if phase != "PRE":
            continue

        # pick active candidates (odds 1.5..8.0)
        cands = _active_candidates_for_market(mid, max_runners=8)
        if not cands: 
            continue
        ...
        for sid, odds in cands:
            ...
            # plan: mastery first, fallback legacy
            try:
                planA = _plan_from_mastery(ctx)
            except Exception:
                planA = None
            if not planA or not planA.get("enter"):
                planA = _legacy_scout_plan(ctx)

            # rulebook (safe): early veto or tighten ticks/size
            preview_code = _next_pair_tag("A", mid)
            skip_this, planA = _apply_rulebook("A", preview_code, ctx, planA)
            if skip_this:
                picked += 1
            ...
            # (keep planA as returned; do NOT fall back yet)


            # per-letter cap (A)
            ok_cap, cap_reason = _can_open_scalp(
                mid, sid, max_per_runner=3, run_id=run_id, family_letter="A"
            )
            if not ok_cap and (ctx.get("source", "LIVE") != "TEST"):
                logger(f"[A] skip cap {cap_reason} mid={mid} sid={sid}")
                picked += 1
                if picked >= MAX_RUNNERS_PER:
                    break
                continue
    ...
    try:
        ...
        for (mid_s, _tto_s) in pre_near[:MAX_MARKETS_S]:
            if not _is_today_in_scope(mid_s):
                logger(f"[S] skip stale/non-today mid={mid_s}")
                continue
            ...
            for (sid_s, odds_s) in cands:
                ...
                # ---------- per-pass rulebook for S (safe, in-loop) ----------
                preview_code = _next_pair_tag("S", mid_s)
                skip_this, plan = _apply_rulebook("S", preview_code, ctx, plan)
                if skip_this:
                    picked += 1
                ...
                # fallback if plan is still non-entering (keeps in-loop flow legal)
                if not plan or not plan.get("enter"):
                    plan = _legacy_scout_plan(ctx)
                if not plan or not plan.get("enter"):
                    picked += 1
                ...
                try:
                    ...
                    try:
                        con = _auto_conn()
                        _exec_retry(con, "UPDATE orders SET source=?, notes=? WHERE id=?", (code, pass_tag or code, parent_id))
                        _commit_retry(con)
                    except Exception:
                        pass
                    ...
                    if ctx.get("source","LIVE") == "LIVE":
                        try:
                            ...
                            con2 = _auto_conn()
                            _q_retry(con2, "UPDATE orders SET bf_bet_id=?, customer_ref=? WHERE id=?",
        ...
        scope2 = _build_scope(now_utc=utcnow(), inplay_window_min=15)
        if not scope2.get("pre_near"):
            return None   # don't place S off context alone when no PRE-near scope
        ...
        # ---------- per-pass rulebook for S (no loop here → early returns) ----------
        preview_code = _next_pair_tag("S", mid_s)
        skip_this, plan = _apply_rulebook("S", preview_code, ctx, plan)
        if skip_this:
            return None
        ...
        if not plan or not plan.get("enter"):
            _gate_bump("no_plan") 
            logger(f"[DEC] NO-TRADE | {plan.get('why','(no reason)') if isinstance(plan,dict) else '(no plan)'}")
            return None
        ...
        if ctx.get("source","LIVE") == "LIVE":
            try:
                ...
                con = _auto_conn()
                _q_retry(con, "UPDATE orders SET bf_bet_id=?, customer_ref=? WHERE id=?",
                            (bet_id, cref, parent_id))
                con.commit()
            except Exception as e:
        ...
        try:
            globals()["_SCOPE_CURSOR"] = (int(globals().get("_SCOPE_CURSOR", 0)) + 1) % max(1, int(n) if "n" in locals() else 1)
        except Exception:
            globals()["_SCOPE_CURSOR"] = 0

        # --- end-of-call heartbeat: print once if nothing placed -----------------
        try:
            if _dbg.get("placed", 0) == 0:
                scope_ok = 0
                try:
                    # quick sense of scope size for visibility
                    sc = _build_scope(now_utc=_utcnow(), inplay_window_min=15)
                    scope_ok = len(sc.get("pre_near", [])) + len(sc.get("in_play", []))
                except Exception:
                    pass
                logger(
                    "[DECDBG] no-entry | scope_ok=%d mkt | cand=%d | gates: "
                    "out_of_band=%d inactive=%d cap=%d rule=%d veto=%d price=%d"
                    % (
                        scope_ok,
                        _dbg.get("cand_seen", 0),
                        _dbg.get("out_of_band", 0),
                        _dbg.get("inactive", 0),
                        _dbg.get("cap", 0),
                        _dbg.get("rule", 0),
                        _dbg.get("veto", 0),
...
def _orders_cols(con: sqlite3.Connection) -> list[str]:
    return [r["name"] for r in _q_retry(con, "PRAGMA table_info(orders)")]
...
def _eod_report(run_id: str, run_day: int, month_index: int, logger=None) -> None:
    ...
    ledger_sum = 0.0
    if _q_retry(db, "SELECT 1 FROM sqlite_master WHERE type='table' AND name='pnl_daily'").fetchone():
        has_src_ledger = any(r[1]=='source' for r in _q_retry(db, "PRAGMA table_info('pnl_daily')"))
        if has_src_ledger:
...
def _snapshot_runner_activity(marketId: str, selectionId: str,
    ...
    try:
        db = _bets_conn()
        db.row_factory = sqlite3.Row
        if _q_retry(db, "SELECT 1 FROM sqlite_master WHERE type='table' AND name='runner_activity'").fetchone():
            row = _q_retry(db, "SELECT status FROM runner_activity WHERE marketId=? AND selectionId=? ORDER BY id DESC LIMIT 1",
                (str(marketId), str(selectionId))
            ).fetchone()
    ...
    try:
        if (last_status is not None) and (last_status != status_now):
            ...
            # NEW: promote market on meaningful signal transitions (extra beyond 5)
            if status_now == "active":
                promote_market_on_signal(str(marketId), ttl_s=180)  # 3 min boost
            elif status_now == "passive":
                promote_market_on_signal(str(marketId), ttl_s=60)   # 1 min softer boost
    except Exception:
        pass
    ...
    try:
        ...
        """)
        _q_retry(db, "INSERT INTO runner_activity(marketId,selectionId,status,created_at) "
          "VALUES (?,?,?, datetime('now','utc'))",
...
def _runner_activity(marketId: str, selectionId: str) -> str:
    ...
    try:
        if _q_retry(db, "SELECT 1 FROM sqlite_master WHERE type='table' AND name='runner_activity'").fetchone():
            r = _q_retry(db, "SELECT status FROM runner_activity WHERE marketId=? AND selectionId=? "
                "ORDER BY rowid DESC LIMIT 1", (str(marketId), str(selectionId))
            ).fetchone()
            if r and r["status"]:
                return str(r["status"]).lower()

        if _q_retry(db, "SELECT 1 FROM sqlite_master WHERE type='table' AND name='inbound_bets_min'").fetchone():
            cols = [c["name"] for c in _q_retry(db, "PRAGMA table_info(inbound_bets_min)")]
            if "active" in cols:
                r = _q_retry(db, "SELECT active FROM inbound_bets_min WHERE marketId=? AND selectionId=? "
                    "ORDER BY rowid DESC LIMIT 1", (str(marketId), str(selectionId))
                ).fetchone()
                if r is not None:
                    return "active" if int(r[0] or 0) == 1 else "ignored"

        if _q_retry(db, "SELECT 1 FROM sqlite_master WHERE type='table' AND name='runners'").fetchone():
            cols = [c["name"] for c in _q_retry(db, "PRAGMA table_info(runners)")]
            if "status" in cols:
                r = _q_retry(db, "SELECT status FROM runners WHERE marketId=? AND selectionId=? "
                    "ORDER BY rowid DESC LIMIT 1", (str(marketId), str(selectionId))
                ).fetchone()
                if r and r["status"]:
                    return str(r["status"]).lower()
...
def _open_parents_count(marketId: str, selectionId: str, run_id: str | None = None) -> int:
    ...
    try:
        ...
        row = _q_retry(con, f"SELECT COUNT(*) AS n FROM orders {where}", tuple(args)).fetchone()
...
def _can_open_scalp(market_id: str,
                    ...
                    *,
                    ...
                    inplay_window_min: int = 15) -> tuple[bool, str]:
    """
    Cap rule (applies to **each letter** independently when family_letter is given):
      Count OPEN parents for THIS (marketId,selectionId) and THIS letter (orders.source LIKE '{letter}%')
      that:
        - were opened TODAY (UTC),
        - are in markets still IN-SCOPE today (future OFF OR OFF <= inplay_window_min ago),
        - and have NO matched child yet.
    """
    import sqlite3
    from engines.config_paths import autoscalp_db
    ...
    con = None
    try:
        src_mode = (mode or _current_source().upper())
        if src_mode not in ("TEST","LEARNING","LIVE"):
            src_mode = "LIVE"
        ...
        # Parent predicate (PARENT = no link or role='PARENT')
        if role_col:
            parent_pred = "p.role='PARENT'"
        elif link:
            parent_pred = f"(p.{link} IS NULL OR p.{link}='')"
        ...
        where_parts = [
            ...
            "p.selectionId=?",
            "date(p.opened_at)=date('now','utc')",
            # in-scope today: future OFF OR OFF within <= inplay_window_min minutes
            f"""
            EXISTS (
              SELECT 1
              FROM markets_schedule s
              WHERE s.marketId = p.marketId
                ...
                AND (
                      datetime(replace(replace(s.off_at_utc,'T',' '),'Z','')) >= datetime('now','utc')
                   OR ((julianday('now','utc') - julianday(datetime(replace(replace(s.off_at_utc,'T',' '),'Z','')))) * 1440.0) <= {int(inplay_window_min)}
                )
            )
            """
        ]
        ...
        # Per-letter constraint
        if src_col and isinstance(family_letter, str) and family_letter.strip():
            where_parts.append("UPPER(COALESCE(p.source,'')) LIKE UPPER(?)")
            args.append(f"{family_letter.strip().upper()}%")

        # Mode (LIVE/TEST/LEARNING) if present
        if mode_col:
            where_parts.append("UPPER(COALESCE(p.mode,''))=UPPER(?)")
            args.append(src_mode)

        # Optional: restrict to run_id if column exists and provided
        if run_id and "run_id" in cols:
            where_parts.append("p.run_id=?")
            args.append(str(run_id))

        where_sql = " AND ".join(where_parts)

        # Count OPEN parents (no matched child)
        if link:
            sql = f"""
...
def check_and_close(order_id: int, direction: str, entry_odds: float, target_ticks: int, stake: float,
    ...
    con = _auto_conn()
    link_col = _ensure_orders_link_col(con)
    row = _q_retry(con, f"SELECT id FROM orders WHERE {link_col}=? ORDER BY id DESC LIMIT 1",
        (order_id,)
    ).fetchone()
    child_id = int(row["id"]) if row else None
    if child_id is not None:
        _q_retry(con, "UPDATE orders SET entry_status='matched', closed_at=datetime('now','utc') WHERE id=?", (child_id,))
    _q_retry(con, "UPDATE orders SET entry_status='matched', closed_at=datetime('now','utc') WHERE id=?", (order_id,))
