...
def status_once(key: str, ok: bool, detail: str = "") -> None:
    try:
        _S = status_once.__dict__.setdefault("_S", {})  # type: ignore[attr-defined]
...
# ── DB connectors (tolerant, accept both ro/rw keywords) ─────────────────────

def open_auto_db(*, ro: Optional[bool] = None, rw: Optional[bool] = None, **_kwargs):
    """Return sqlite3.Connection or None. Accepts either ro/rw flags.
    Preference order: ro=True -> mode=ro, else rw=True -> mode=rw, else ro by default.
    """
    try:
        path = _path_call("autoscalp_db")
        if not path:
            return None
        mode = "ro" if (ro is True or rw is not True) else "rw"
...
def open_bets_db(*, ro: Optional[bool] = None, rw: Optional[bool] = None, **_kwargs):
    try:
        path = _path_call("bets_db")
        if not path:
            return None
        mode = "ro" if (ro is True or rw is not True) else "rw"
        uri = f"file:{path}?mode={mode}&cache=shared"
...
# Legacy aliases some code calls
...
def _active_candidates_for_market(mid: str, max_runners: int = 8) -> List[Tuple[str, float]]:
    ...
    if not odds:
        ...
        return []
    pool = [(sid, px) for sid, px in odds.items() if _runner_is_active(mid, sid, px)]
    if not pool:
        return []
    try:
        ...
        cur_ids = [sid for (sid, _px) in sorted(pool, key=lambda t: (float(t[1]), str(t[0])))]
        ...
        pairs = [(sid, odds.get(sid)) for sid in seq if sid in odds]
        if orders.get(mid):
        ...
        return sorted(pool, key=lambda t: (float(t[1]), str(t[0])))[:max_runners]
