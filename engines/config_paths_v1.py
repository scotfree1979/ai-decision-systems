...
def _get_local_conn(fam: str):
    """Return SINGLETON local RW connection for this family."""
    lock = _CONN_LOCK[fam]
    with lock:
        con = _LOCAL_CONN[fam]
        if con is None:
...
def _get_cloud_conn(fam: str):
    """Return SINGLETON cloud RW connection for this family."""
    lock = _CONN_LOCK[fam]
    with lock:
        con = _CLOUD_CONN[fam]
        if con is None:
...
def open_auto_db(*, rw=None, **_):
    """AUTO family connector (cached)."""
    fam = "auto"

    # 🔥 FIX #1: SETUP = ALWAYS LOCAL ONLY
    if DAL_MODE.upper() == "SETUP":
        return _get_local_conn(fam)

    # LIVE → cloud write
    con = _get_cloud_conn(fam)
    _attach_all_four(con, rw_primary=fam)
    return con
...
def open_db(family: str, ro=False, rw=False, **_):
    """AlphaX entrypoint → routed through cached DAL."""
