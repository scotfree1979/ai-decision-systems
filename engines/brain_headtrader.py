...
try:
    ...
    print("✅ launch_nextgen_thread successfully imported.")
except ImportError as e:
    print(f"❌ Import failed for launch_nextgen_thread: {e}")
    launch_nextgen_thread = None
...
def evaluate_signal(horses=live_runner_map):
    ...
    def finalise_signal(signal):
        ...
        try:
            ...
            if signal.get("status") == "approved":
                ...
                # ✅ Use NextGen if available
                if launch_nextgen_thread:
                    launch_nextgen_thread(signal)
        except Exception as e:
    ...
    safe_runner_map = copy.deepcopy(live_runner_map)

    for market_id, selection_id in list(rows):
        ...
        try:
            ...
            safe_runner_map.setdefault(market_id, []).append(runner)
    ...
    for market_id, selection_id in rows:
        try:
            ...
            # ✅ Inject position and minutes_to_post into signal (for use in signal memory + NextGen filters)
            for runner in horses:
                if runner["selectionId"] == selection_id:
                    signal["position"] = runner.get("position")
                    signal["minutes_to_post"] = runner.get("minutes_to_post")
                    break
