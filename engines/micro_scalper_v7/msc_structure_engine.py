...
class StructureEngine:
    ...
    def __init__(self):
        ...
        # --------------------------------------------------
        # 🔑 CONTEXT STORE BIND (FROM CONTEXT ENGINE)
        # --------------------------------------------------
        try:
            from engines.bus.bus import BUS
            ctx_engine = BUS.engines.get("MSC_CONTEXT")
            self._context_store = getattr(ctx_engine, "_context_store", {})
        except Exception:
            self._context_store = {}
...
# PURPOSE
# -------
# Provide the same context performance probe used during development
# but callable internally by the engine.
#
# DATA SOURCE
# -----------
...
# OUTPUT
# ------
# Context performance grouped by market structure:
#
#   band
#   fav_gap_bucket
#   field_bucket
#   fav_strength
#
# Used later by the context scoring model.
# ======================================================================================================
            ...
            for c in candidates:
                ...
                if 15 <= px <= 20:
                    ...
                    emit_plan({
                        ...
                        "direction": "LAY->BACK",
                        "px": px,
                        "why": "context_inplay_secondary_harvest",
                    })       
        ...
        if not plans:
            ...
            if candidates:
                ...
                for c in candidates:
                    ...
                    plans.append({
                        "enter": True,
                        "engine": "MSC_STRUCTURE",   # replace per engine
                        "role": "PARENT",
                        "marketId": c["marketId"],
