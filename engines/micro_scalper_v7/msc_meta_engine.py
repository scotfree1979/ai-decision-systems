...
# ======================================================================================================
# 📍 TARGET: engines/micro_scalper_v7/msc_blueprint_engine.py
# 🔎 SEARCH: class BlueprintEngine:
# 🧩 ACTION: ADD BELOW IMPORTS
# 📆 PATCHED: Blueprint cache loader
#
# PURPOSE
# -------
# Load OC runner story expectations generated by blueprint builder.
# File generated each morning:
...
def load_blueprint_cache():
    ...
    global BLUEPRINT_CACHE

    try:

        cache_path = Path("data/blueprints/runner_story_oc_cache.json")

        if cache_path.exists():

            with open(cache_path) as f:
                BLUEPRINT_CACHE = json.load(f)
    ...
    return BLUEPRINT_CACHE

# ======================================================================================================
# 📍 TARGET: engines/micro_scalper_v7/msc_blueprint_engine.py
# 🔎 SEARCH: def _build_layer2_surface
# 🧩 ACTION: ADD ABOVE FUNCTION
# ======================================================================================================

def map_minutes_to_oc(tto_seconds):

    if tto_seconds is None:
        return None

    minutes = tto_seconds / 60
    ...
    if minutes >= 80:
        ...
        # in-play minute buckets
        oc = int(abs(minutes)) + 7
        return min(20, oc)
...
# ======================================================================================================
# 📍 TARGET: engines/micro_scalper_v7/msc_blueprint_engine.py
# 🔎 SEARCH: def build_surface_key(ctx):
# 🧩 ACTION: REPLACE FUNCTION
# 📆 PATCHED: 2026-03-17 — standalone safety for missing race metadata
#
# PURPOSE
# -------
# BUS runtime ctx does not always contain race metadata (distance, handicap, race_type).
# Standalone mode therefore produces None values.
#
# This patch protects the Blueprint surface builder so it never crashes.
# ======================================================================================================

def build_surface_key(ctx):

    race_type = ctx.get("race_type")
    distance  = ctx.get("distance")
    handicap  = ctx.get("handicap")
    ...
    return f"{base}_{dist}_{cat}"

# ======================================================================================================
# 📍 TARGET: engines/micro_scalper_v7/msc_blueprint_engine.py
# 🔎 SEARCH: map_minutes_to_oc
# 🧩 ACTION: ADD BELOW
# ======================================================================================================

def score_blueprint_alignment(surface_key, oc_phase, live_move):

    story = BLUEPRINT_CACHE.get(surface_key)
    ...
    expected = oc.get("expected_move")

    if expected == live_move:
        return 3

    if expected in ("drift", "steam"):
        return -2
...
# ======================================================================================================
# 📍 TARGET: engines/micro_scalper_v7/msc_blueprint_engine.py
# 🔎 SEARCH: def _get_market_metadata(self, market_id):
# 🧩 ACTION: REPLACE FUNCTION
# 📆 PATCHED: Fix standalone metadata crash (race_type column does not exist)
#
# ROOT CAUSE
# ----------
# bets table stores race metadata in meta_json, not separate columns.
# Querying race_type/distance/handicap causes:
#     sqlite3.OperationalError: no such column: race_type
#
# FIX
# ---
# Read meta_json and extract fields safely.
# Works in BUS mode and standalone mode.
# ======================================================================================================

    def _get_market_metadata(self, market_id):
...
# ======================================================================================================
    ...
    def _context_learned_score(self, mid, sid):

        key = getattr(self, "_get_context_key", lambda a, b: None)(mid, sid)
        ...
        if not data or data.get("trades", 0) < 10:
            return 0.0

        return data.get("avg_pnl", 0.0)
...
# ======================================================================================================
    ...
    def _build_blueprint_surface(self, timing_surface):
        ...
        for (mid, sid), ctx in getattr(self, "_route_ctx_map", {}).items():
            ...
            tto = market.get("tto_seconds")
            oc_phase = map_minutes_to_oc(tto) if tto is not None else None

            live_move = ctx.get("direction")

# ======================================================================================================
# 📍 TARGET: engines/micro_scalper_v7/msc_blueprint_engine.py
# 🔎 SEARCH: meta = self._get_market_metadata(mid)
# 🧩 ACTION: REPLACE BLOCK
# 📆 PATCHED: Prefer CTX metadata, fallback to DB
#
# PURPOSE
# -------
# Avoid redundant DB lookups.
# BUS already injects meta_json into ctx.
# ======================================================================================================
...
# ======================================================================================================
# 📍 TARGET: engines/micro_scalper_v7/msc_context_engine.py:_build_blueprint_surface
# 🔎 SEARCH: surface_key = build_surface_key
# 🧩 ACTION: ADD — persist blueprint surface into ctx
# 📆 PATCHED: 2026-03-18 — FIX blueprint scoring (ctx wiring)
#
# PURPOSE
# -------
# Layer2 expects ctx["blueprint_surface"].
# Without this, blueprint scoring = ALWAYS ZERO.
#
# FIX
# ---
# Persist surface_key into ctx so downstream scoring works.
# ======================================================================================================

            ctx["blueprint_surface"] = surface_key
...
# 📍 TARGET: engines/micro_scalper_v7/msc_context_engine.py:_build_v7_report
# 🔎 SEARCH: report = {
# 🧩 ACTION: REPLACE — fix invalid dict + add context surfaces
# 📆 PATCHED: 2026-03-18 — FIX syntax + enable META context layer
#
# PURPOSE
# -------
# Previous code had context surfaces OUTSIDE dict → SyntaxError.
#
# FIX
# ---
...
# ======================================================================================================
        ...
        for r in drift:
            ...
            if market:
                oc_phase = map_minutes_to_oc(market.get("tto_seconds"))
...
# ======================================================================================================
            ...
            # --------------------------------------------------
            # Blueprint confidence weighting (later OC = stronger)
            # --------------------------------------------------

            oc_weight = 1.0

            if oc_phase is not None:
...
# ======================================================================================================
    ...
    engine = BUS.engines.get("MSC_META")
