...
def seed_initial_anchor_odds():
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        # 🧠 Check if seeding already occurred today
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS system_flags (
                key TEXT PRIMARY KEY,
                value TEXT
        ...
        if row and row[0] == today:
            print("🔁 Anchor odds already seeded today. Skipping...")
            return
        ...
        for market in markets:
            ...
            if minutes_to_off > 500 or len(market.get("runners", [])) < 7:
                continue

            for runner in market.get("runners", []):
                selection_id = runner["selectionId"]
                odds = fetch_live_odds(session_token, market_id, selection_id)
                if not odds or "lay" not in odds:
                    continue

                lay = odds["lay"]

                cursor.execute("""
                    SELECT anchor_odd FROM bets WHERE marketId = ? AND selectionId = ? AND anchor_odd IS NOT NULL
                """, (market_id, selection_id))
                if not cursor.fetchone():
                    cursor.execute("""
                        INSERT OR REPLACE INTO bets (marketId, selectionId, anchor_odd, placed_at)
                        VALUES (?, ?, ?, ?)
                    """, (market_id, selection_id, lay, datetime.utcnow().isoformat()))
                    seeded += 1

        # 🧾 Record that seeding was done today
        cursor.execute("""
            INSERT OR REPLACE INTO system_flags (key, value) VALUES ('anchor_seeded_date', ?)
        """, (today,))
        conn.commit()

    print(f"🧊 Seeded anchor odds for {seeded} runners at tool launch.")
...
# -----------------------------
# 🧩 Odds Check Filler – Updates all odds_check fields if fallback used
# -----------------------------
def update_odds_checks_with_anchor(market_id, selection_id, anchor):
    import sqlite3
    from config_paths import DB_PATH
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if checks and all(v is None for v in checks):
            cursor.execute("""
                UPDATE bets
                SET odds_check_1 = ?, odds_check_2 = ?, odds_check_3 = ?,
                    odds_check_4 = ?, odds_check_5 = ?, odds_check_6 = ?
                WHERE marketId = ? AND selectionId = ?
            """, (anchor, anchor, anchor, anchor, anchor, anchor, market_id, selection_id))
            conn.commit()
            print(f"🧠 Seeded odds_check_1–6 for {selection_id} @ {anchor}")
        else:
            print(f"ℹ️ Skipped odds check fill for {selection_id} – some fields already set")
...
def market_loop():
    ...
    from config_paths import DB_PATH
    from brain_headtrader import evaluate_signal
    from scalper_coop import fetch_anchor_odds, update_odds_checks_with_anchor
    from datetime import datetime, timezone
    import sqlite3, time

    print("🚀 Starting market loop...")

    last_odds_map = {}  # ✨ Persistent across loops

    while True:
        all_markets = get_markets_and_insert()
        now = datetime.now(timezone.utc)
        filtered = [m for m in all_markets if -15 <= (datetime.fromisoformat(m["marketStartTime"].replace("Z", "+00:00")) - now).total_seconds() / 60.0 <= 100 and len(m.get("runners", [])) >= 7]

        segment_totals = {"<20": 0, "20-40": 0, "40-60": 0, "60-80": 0, "80-90": 0, ">90": 0}
        market_segments = {"<20": 0, "20-40": 0, "40-60": 0, "60-80": 0, "80-90": 0, ">90": 0}
        odds_check_totals = {"oc1": 0, "oc2": 0, "oc3": 0, "oc4": 0, "oc5": 0, "oc6": 0}
        signal_count = 0
        ...
        for market in filtered:
            ...
            if minutes_to_off < 20:
                market_segments["<20"] += 1
            elif 20 <= minutes_to_off < 40:
                market_segments["20-40"] += 1
            elif 40 <= minutes_to_off < 60:
                market_segments["40-60"] += 1
            elif 60 <= minutes_to_off < 80:
                market_segments["60-80"] += 1
            elif 80 <= minutes_to_off <= 90:
                market_segments["80-90"] += 1
            else:
                market_segments[">90"] += 1
            ...
            for runner in market.get("runners", []):
                ...
                # 🧠 Always update odds checks if empty – use anchor fallback if needed
                ...
                with sqlite3.connect(DB_PATH) as conn:
                    ...
                    oc_field = None
                    field_key = None
                    if 85 <= minutes_to_off <= 95:
                        oc_field, field_key = "odds_check_1", "oc1"
                    elif 65 <= minutes_to_off < 85:
                        oc_field, field_key = "odds_check_2", "oc2"
                    elif 45 <= minutes_to_off < 65:
                        oc_field, field_key = "odds_check_3", "oc3"
                    elif 25 <= minutes_to_off < 45:
                        oc_field, field_key = "odds_check_4", "oc4"
                    elif 10 <= minutes_to_off < 25:
                        oc_field, field_key = "odds_check_5", "oc5"
                    elif 0 <= minutes_to_off < 10:
                        oc_field, field_key = "odds_check_6", "oc6"

                    if oc_field:
                        cursor.execute(f"SELECT {oc_field} FROM bets WHERE marketId = ? AND selectionId = ?", (market_id, selection_id))
                        existing_value = cursor.fetchone()
                        if existing_value and existing_value[0] is None:
                            cursor.execute(f"UPDATE bets SET {oc_field} = ? WHERE marketId = ? AND selectionId = ?", (current_odds, market_id, selection_id))
                            conn.commit()
                            odds_check_totals[field_key] += 1

                if 0 <= minutes_to_off < 20:
                    ...
                    if not range_low or not range_high or range_low >= range_high:
                        print(f"⚠️ Skipped signal for {runner['runnerName']} – invalid volatility range")
                        continue
                    ...
                    for r in market_runners:
                        r["odds"] = last_odds_map.get(r["selectionId"], None)
        ...
        print("📈 Market Segment Breakdown:", market_segments)
        print("📈 Runner Segment Totals:", segment_totals)
        print("📊 Odds Check Summary:")
        for key, value in odds_check_totals.items():
            print(f"  • {key.upper()} updated: {value}")
        print(f"📊 Signals Generated This Loop: {signal_count}")
...
def run_coop_session():
    ...
    while True:
        ...
        # ✅ FIXED: Only switch to test mode if *all* markets are more than 90 mins away
        future_markets = [
            m for m in all_markets
            if (datetime.fromisoformat(m["marketStartTime"].replace("Z", "+00:00")) - now).total_seconds() / 60.0 > 90
            and len(m.get("runners", [])) >= 7
        ]

        if len(future_markets) == len(all_markets):
            print(f"🧪 {len(future_markets)} future markets, {len(all_markets)} total. Switching to test mode only if all are future.")
