...
def calculate_hedge_stake(entry_odds, entry_stake, hedge_odds, direction):
    if direction == "lay_to_back":
        return round((entry_odds * entry_stake) / hedge_odds, 2)
    elif direction == "back_to_lay":
        return round((entry_stake * hedge_odds) / entry_odds, 2)
    else:
        raise ValueError(f"❌ Unknown direction '{direction}' in hedge stake calc")
...
def save_bet_to_db(market_id, selection_id, odds, stake, side, ref, bet_id):
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            # ✅ Save to live_signals (main tracking table)
            cursor.execute("""
                INSERT INTO live_signals (
                    marketId, selectionId, odds, stake, side, customerOrderRef, bet_id, placed_at
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
            ...
            # ✅ Also save to bets table (audit + strategy logging)
            cursor.execute("""
                INSERT INTO bets (
                    marketId, selectionId, odds, stake, side, customerOrderRef, bet_id, placed_at
                )
...
def launch_nextgen_thread(signal):
    ...
    from signal_memory_engine.relay import relay_signal_to_memory  # ✅ NEW IMPORT

    signal = signal_memory.enrich(signal)
    ...
    if not signal.get("blueprint_match"):
        if conf >= 0.76 and signal.get("tick_pattern") in ["drifted", "steamed"]:
            print(f"🔍 High-confidence drift/steam accepted | Confidence: {conf}")
        elif 0.70 <= conf < 0.76 and signal_memory.is_trend_forming(
            signal["marketId"], signal["selectionId"], signal.get("scalp_direction"),
            signal.get("odds"), signal.get("range_high"), signal.get("range_low")):
            print(f"🔄 Trend forming – non-blueprint trade allowed | Confidence: {conf}")
        elif signal.get("tick_pattern") == "flat" and 0.68 <= conf < 0.72 and tick_range >= 8:
            print("🔁 Rebound candidate detected – flat pattern.")
            signal["signal_type"] = "rebound_drift"
        else:
            relay_signal_to_memory(signal)  # ✅ NEW DB relay instead of direct injection
            print(f"📤 SIGNAL RELAYED → NextGen: {signal.get('selectionId')} | Confidence: {signal.get('confidence')} | Pattern: {pattern_key}")
            return
...
def place_bet_with_retries(
    ...
    for attempt in range(max_retries + 1):
        ...
        response = place_bet(session_token, market_id, selection_id, stake, odds, side, ref)
        
        if isinstance(response, dict) and "result" in response:
            try:
                bet_id = response["result"]["instructionReports"][0]["betId"]
                print(f"✅ Bet placed: {side} £{stake} @ {odds} | Ref: {ref} | BetID: {bet_id}")
                save_bet_to_db(market_id, selection_id, odds, stake, side, ref, bet_id)
                return bet_id
            except Exception as e:
                print(f"❌ Failed to extract betId: {e}")
                continue
...
def run_nextgen_scalp(signal):
    ...
    try:
        ...
        # 🔍 Infer direction from blueprint pattern if needed
        pattern_key = signal.get("pattern_key", "")
        direction = "back_to_lay" if pattern_key.startswith("steam") else "lay_to_back"

        if direction == "lay_to_back":
            entry_side = "LAY"
            hedge_side = "BACK"
            hedge_odds = walk_ticks(odds, ticks, direction="up")
        else:
            entry_side = "BACK"
            hedge_side = "LAY"
            hedge_odds = walk_ticks(odds, ticks, direction="down")

        suffix = _next_suffix()
        base_ref = f"NG_{suffix}"
        entry_ref = f"{base_ref}_E"
        hedge_ref = f"{base_ref}_H"
        hedge_sub_ref = f"{hedge_ref}_P0"
        ...
        hedge_bet_id = place_bet(
            ...
            odds=hedge_odds,
            bet_type=hedge_side,
            customer_order_ref=hedge_sub_ref
        )

        if entry_bet_id and hedge_bet_id:
            ...
            signal_memory.record_matched_bet(market_id, selection_id, entry_side, odds, stake)
            signal_memory.record_matched_bet(market_id, selection_id, hedge_side, hedge_odds, hedge_stake)
            resolved = signal_memory.resolve_runner_pnl(market_id, selection_id)

            if not resolved:
                print(f"❌ [NEXTGEN] resolve_runner_pnl failed for ({market_id}, {selection_id})")
            else:
                print(f"✅ [NEXTGEN] resolve_runner_pnl completed successfully.")

            # ✅ Calculate current trade profit/loss explicitly
            entry_liability = signal_memory.estimate_liability(entry_odds, stake, entry_side)
            hedge_liability = signal_memory.estimate_liability(hedge_odds, hedge_stake, hedge_side)
            trade_pnl = round(hedge_liability - entry_liability, 2)

            # ✅ DECREMENT SAFEGUARD
            if active_scalps[selection_id] < 0:
                print(f"⚠️ active_scalps went negative for {selection_id}. Resetting to 0.")
                active_scalps[selection_id] = 0

            # ✅ Warn on negative P&L
            if trade_pnl < 0:
                print(f"⚠️ [WARNING] Negative trade P&L on {selection_id}: £{trade_pnl:.2f}")
...
📏 Tick Count:       {ticks}
📐 Tick Size:        {tick_size}
📊 Range (Lo→Hi):    {signal.get("range_low")} → {signal.get("range_high")}
🔄 Position Ratio:   {signal.get("position_ratio")}
📈 Tick Pattern:     {signal.get("tick_pattern")}
🔥 Confidence:       {signal.get("confidence")}
🧠 Signal Type:      {signal.get("signal_type")}
🧬 Blueprint Match:  {signal.get("blueprint_match") or 'None'}
🧩 Pattern Key:      {signal.get("pattern_key") or 'None'}
📖 Playbook Ref:     {signal.get("customerOrderRef") or 'N/A'}
💹 Trade P&L:        £{trade_pnl:.2f}
💼 Balance After:    £{signal_memory.current_balance:.2f}
📊 Session P&L:      £{(signal_memory.current_balance - signal_memory.starting_balance):+.2f}
📈 Total Trades:     {len(signal_memory.trade_log)}
""")

        else:
            if entry_bet_id:
                cancel_bet(session_token, entry_bet_id)
            print("❌ Trade setup incomplete — one side failed. Aborting.")
            signal_memory.record_signal_outcome(
                market_id, selection_id, result="partial",
                explanation="One side of the trade failed to place"
            )
