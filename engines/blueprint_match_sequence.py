...
def predict_next_segment(sequence_so_far, known_patterns):
    """
    Given a partial sequence (e.g. ['drift→OC1–OC2', 'drift→OC2–OC3']),
    scan known blueprints and predict the most likely next move.
    """
    candidates = []

    for pattern_key, data in known_patterns.items():
        pattern_seq = data.get("sequence", [])
        for i in range(len(pattern_seq) - len(sequence_so_far)):
            window = pattern_seq[i:i+len(sequence_so_far)]
            if window == sequence_so_far:
                next_segment = pattern_seq[i+len(sequence_so_far)]
                candidates.append(next_segment)

    if not candidates:
        return None

    # Return most common next segment
